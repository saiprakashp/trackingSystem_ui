...
import {loginUser} from '../../MasterPages/store/actions/indexAction';
...
const Login = (props) => {
    ...
    const loginuser = () => {
        ...
        if (userName != null && userName !== '' && password != null && password !== '') {
            props.onLoginUser('userName', userName,
                null, null, 'password', password,
                props.history);
        } else {
    ...
    const forgetPassword = () => {
        props.history.push('/pts-ui/forgetPassword');
    };
                    ...
                    <div className='row '>
                        ...
                        <div className='col-sm-12 col-md-7'>
                            <a onClick={forgetPassword}
                               className=" text-left float-leftt text-primary ">{title.FORGET_PASSWORD}
                            </a>
                            <button type="button" onClick={loginuser}
                                    className="btn  float-right mr-3 btn-sm btn-primary">{title.LOGIN}
                            </button>
                        </div>
                    </div>
                </form>
...
const mapDispatchtoProps = dispatch => {
    return {
        onLoginUser: (userKey, userValue, secureKey,
                      secureValue, passwordKey, passwordValue,
                      history) => {
            dispatch(loginUser(userKey, userValue, passwordKey,
                passwordValue, secureKey, secureValue, history))
        }
    }
};
